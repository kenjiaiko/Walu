...
　ラベルが付けばあとは適当なアルゴリズムで学習させれば良い。課題はいかに「ラベル付け」を自動化できるか。今回は最後の742行を人間が確認したが、これを完全に自動化できればデータセットから攻撃のみを検知できる。もちろんIsolation forestだけ（というより単一のアルゴリズムだけ）では無理だろうが、その解決法を探すことが課題。
